...
const Contact = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const newErrors = {};
        if (!fullName.trim()) {
            newErrors.fullName = 'Ad Soyad';
        }
        if (!email.trim()) {
            newErrors.email = 'Email';
        }
        if (phone === undefined || !phone.trim()) {
            newErrors.phone = 'Telefon';
        }
        setErrors(newErrors);
        if (Object.keys(newErrors).length === 0) {
            if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) {
                ...
                try {
                    const response = await fetch("https://api.metalab.az/api/forms", {
                        method: "POST",
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        setErrors({ error: false, message: "Form uğurla göndərildi." });
                    } else {
                        setErrors({ error: true, message: "Xəta baş verdi. Zəhmət olmasa yenidən cəhd edin." });
                    }
                } catch (error) {
            ...
            setErrors((prev) => ({
                ...prev,
                message: `${Object.values(newErrors).join(", ")} doldurulmalıdır.`
            }))
        }
    };
            ...
            <div className="container relative">
                <div className="row justify-between gap-y-20">
                    <div className="col-lg-6">
                        <h1 className="text-white w-[80%] font-semibold mb-[100px] xl:mb-[50px]">Bizimlə Əlaqə.</h1>
                        <div className="contact-box bg-[linear-gradient(135deg,rgba(216,216,216,0.2),transparent)] rounded-[35px] px-[55px] py-[70px] backdrop-blur-2xl backdrop-saturate-100 xl:px-[45px] xl:py-[60px] lg:py-[40px] lg:px-[30px]">
                            <h2 className="heading-4 text-white font-semibold mb-[55px] xl:mb-[30px]">Bizimlə Əlaqə.</h2>
                    ...
                    <div className="col-auto">
                        <div className="flex items-start mb-[90px]">
                            ...
                            <div>
                                <h3 className="heading-4 text-white font-semibold mb-[10px]">Telefon</h3>
                                <a href="tel:+994708713641" className="heading-6">+994 (70) 871 36 41</a>
                            </div>
                        </div>
                        <div className="flex items-start mb-[90px]">
                            <Image className="mr-[25px] mt-[5px]" width={35} height={35} src="/assets/icons/mail.svg" alt="Mail icon" />
                            <div>
                                <h3 className="heading-4 text-white font-semibold mb-[10px]">Email</h3>
                                <a href={`mailto: "info@metalab.az"`} className="heading-6">info@metalab.az</a>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start">
                            ...
                            <div>
                                <h3 className="heading-4 text-white font-semibold mb-[10px]">Bizi Sosial Media-da izləyin</h3>
                                <ul className="flex items-center gap-x-[25px]">
                                    <li><Link href="https://www.facebook.com/rEvanzzilla" target="_blank" className="hover:opacity-50 transition-all"><Image width={17} height={17} src="/assets/icons/facebook.svg" alt="facebook icon" /></Link></li>
                                    <li><Link href={"https://wa.me/994708713641"} target="_blank" className="hover:opacity-50 transition-all"><Image width={24} height={24} src="/assets/icons/whatsapp.svg" alt="twitter icon" /></Link></li>
